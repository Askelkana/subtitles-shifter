...
(function () {
	$(function () {
		$('#calculate-delta').click(function () {
			var movieTimestamp = (parseInt($('#movie-timestamp-hour').val() || 0, 10) * 3600 +
				parseInt($('#movie-timestamp-minute').val() || 0, 10) * 60 +
				parseInt($('#movie-timestamp-second').val() || 0, 10)) * 1000 +
				parseInt($('#movie-timestamp-millisecond').val() || 0, 10);
			var subtitlesTimestamp = (parseInt($('#subtitles-timestamp-hour').val() || 0, 10) * 3600 +
				parseInt($('#subtitles-timestamp-minute').val() || 0, 10) * 60 +
				parseInt($('#subtitles-timestamp-second').val() || 0, 10)) * 1000 +
		...
		$('#make-output').click(function () {
			var parts = $('#input').val().match(/(.+\\)[^\\]*/);
			if (parts && parts.length > 1) {
				$('#output').val(parts[1] + 'subs.srt');
			}
		});
	});
})();
